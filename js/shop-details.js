        ...
        productDetailsHTML = `
                    <div class="product-details__content d-flex">
                        <div class="product-details__content-left position-relative" style=" padding: 60px 100px">
                            ...
                            <div class="product-details__content-left__img-wrapper"> <img src=${dataProductAll[i].imgUrl
            } alt="bala"
                        ...
                        <div class="product-details__content-right  d-flex flex-column justify-content-center "
                            ...
                            <h3 class="product-details__name pb-3">${dataProductAll[i].name
            }</h3>
                            <div class="mb-1" style="color:#bbb;font-size:14px">Categories:
                               <span class="text-black"> ${dataProductAll[i].categories
            }</span>
                            </div>
                            <div class="mb-5" style="color:#bbb;font-size:14px">Brand: <span class="text-black">${dataProductAll[i].brand
            }</span></div>
                            <div class="product-details__desc pb-5" style="max-width:80%">${dataProductAll[i].desc
            }</div>
                            ...
                            <div class="product-details__actions p-5 border rounded d-flex align-items-center gap-5" style="max-width: 90%;
                                ...
                                <i class="fa-solid fa-cart-shopping  cart-in-product ${(storageCartValue || []).includes(
                dataProductAll[i].code
            )
                ? " cart-selected"
                : ""
            }" data-codeProduct=${dataProductAll[i].code
            } style="cursor: pointer; padding: 15px; background-color: #fff; border: 1px solid rgb(192, 190, 190); border-radius: 50%;"></i>
                                <i style="cursor: pointer; padding: 15px; background-color: #fff; border: 1px solid rgb(192, 190, 190); border-radius: 50%;" class="fa-regular fa-heart  ${(storageFavoriteValue || []).includes(
                dataProductAll[i].code
            )
                ? " heart-selected"
                : ""
                            ...
                            <div class="product__group-prices p-5 mt-2 sale__price font-25px">
                                <p class=${dataProductAll[i].discount > 0
                ? "real__price real__price-details"
                : "normal__price normal__price-details"
            }>$${dataProductAll[i].price.toFixed(2)}</p>
                                <p class="sale__price sale__price-details ${dataProductAll[i].discount > 0
                ? ""
                : " d-none"
            }">$${(
